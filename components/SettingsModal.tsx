    ...
    <div className="fixed inset-0 z-50 bg-black flex flex-col overflow-hidden animate-fade-in">
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar p-8 pb-32">
          <div className="max-w-xl mx-auto">
            ...
            <p className="text-green-500/40 text-xs font-black uppercase tracking-[0.3em] mt-2">Refine your operational edge</p>
        </div>
        
        {/* Cycle Duration */}
        <div className="mb-10">
             <div className="flex items-center justify-between mb-4">
                <span className="text-white font-black uppercase text-sm tracking-[0.2em] border-l-4 border-green-500 pl-4">Cycle Length</span>
                <span className="text-green-500 font-black">{duration}m</span>
             </div>
        ...
        <div className="mb-10">
          <div className="flex items-center justify-between mb-6">
            <span className="text-white font-black uppercase text-sm tracking-[0.2em] border-l-4 border-green-500 pl-4">Active Duty</span>
            <button 
          ...
          <div className={`bg-white/5 rounded-3xl p-6 space-y-6 border border-white/5 shadow-inner transition-opacity ${localSchedule.enabled ? 'opacity-100' : 'opacity-30 pointer-events-none'}`}>
            ...
            {/* Days */}
            <div>
              <label className="text-xs uppercase font-black text-white/30 block mb-3 tracking-[0.2em]">Battlegrounds</label>
            ...
            <div className="flex gap-4">
               <div className="flex-1">
                  <label className="text-xs uppercase font-black text-white/30 block mb-2 tracking-[0.2em]">Start Ops</label>
                  <input 
                    type="time" 
                    value={localSchedule.startTime}
                    onChange={(e) => setLocalSchedule(p => ({ ...p, startTime: e.target.value }))}
                    className="w-full bg-black/40 text-white font-black text-sm rounded-xl px-4 py-4 border border-white/10 outline-none focus:border-green-500/50 transition-all text-center"
                  />
               </div>
               <div className="flex-1">
                  <label className="text-xs uppercase font-black text-white/30 block mb-2 tracking-[0.2em]">End Ops</label>
                  <input 
                    type="time" 
        ...
        <div className="mb-10">
             <div className="flex items-center justify-between mb-4">
                <span className="text-white font-black uppercase text-sm tracking-[0.2em] border-l-4 border-green-500 pl-4">Tactical AI</span>
             </div>
             <button 
        ...
        <div className="mb-10">
            <div className="flex items-center justify-between mb-6">
                <span className="text-white font-black uppercase text-sm tracking-[0.2em] border-l-4 border-green-500 pl-4">Stand Down</span>
            </div>
            ...
            {breakUntil && breakUntil > Date.now() ? (
                <div className="bg-green-500/10 border border-green-500/20 rounded-3xl p-6 text-center space-y-4">
                    ...
                    <p className="text-2xl font-black text-white">{new Date(breakUntil).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})}</p>
                    <button 
                ...
                <div className="grid grid-cols-2 gap-3">
                    {[
                        ...
                        <button 
                            ...
                            className="py-4 bg-zinc-900 border border-zinc-800 hover:bg-zinc-800 hover:border-zinc-700 rounded-2xl text-xs font-black uppercase text-white/60 hover:text-white transition-all active:scale-95"
                        >
                            {opt.label}
                        </button>
                    ))}
        ...
        <div className="space-y-4">
            ...
            <button 
                ...
                className="w-full text-xs font-black uppercase tracking-[0.2em] text-white/20 hover:text-white flex items-center justify-center gap-2 py-4 transition-colors"
            >
            ...
            {showTroubleshoot && (
                <div className="bg-black/60 rounded-2xl p-6 text-sm text-white/40 space-y-4 border border-white/5 animate-fade-in">
                    <div>
                        <strong className="text-white block mb-1 font-black uppercase tracking-widest text-xs">1. COMMS CHANNEL</strong>
                        <p className="mb-3">Ensure tactical notifications are authorized.</p>
                        <button onClick={checkPermissions} className="text-xs bg-white/10 hover:bg-white/20 px-4 py-2 rounded-lg text-white font-black uppercase tracking-widest transition-colors">
                            RE-AUTHORIZE
                        </button>
                    </div>
                    <div>
                        <strong className="text-white block mb-1 font-black uppercase tracking-widest text-xs">2. POWER MANAGEMENT</strong>
        ...
        <div className="grid grid-cols-2 gap-4">
            ...
            >
                <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round" className="text-white/20 group-hover:text-green-500 transition-colors"><circle cx="18" cy="5" r="3"></circle><circle cx="6" cy="12" r="3"></circle><circle cx="18" cy="19" r="3"></circle><line x1="8.59" y1="13.51" x2="15.42" y2="17.49"></line><line x1="15.41" y1="6.51" x2="8.59" y2="10.49"></line></svg>
                <span className="text-xs font-black uppercase text-white/20 group-hover:text-white tracking-[0.2em] transition-colors">Invite Friends</span>
            </button>
             <button 
                onClick={handleRate}
                className="bg-white/5 hover:bg-white/10 p-6 rounded-2xl flex flex-col items-center gap-3 transition-all active:scale-95 group border border-white/5"
            >
                <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round" className="text-white/20 group-hover:text-green-500 transition-colors"><polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2"></polygon></svg>
                <span className="text-xs font-black uppercase text-white/20 group-hover:text-white tracking-[0.2em] transition-colors">Review</span>
            </button>
        </div>
        ...
        <div className="p-8 pt-6 border-t border-white/5 bg-[#0a0a0a] z-10">
            ...
            <button
              ...
              className="w-full py-5 rounded-xl font-black uppercase text-white/20 hover:text-white transition-colors tracking-[0.3em] text-sm"
            >
              CLOSE COMMAND
            </button>
            </div>

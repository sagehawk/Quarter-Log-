      ...
      {Object.keys(groupedLogs).map(date => (
        <div key={date} className="relative">
          <div className="mb-6 flex items-center gap-4">
             <span className="text-xs font-black uppercase tracking-[0.3em] text-yellow-500/40 italic">
              {date}
             </span>
          ...
          <div className="space-y-4">
            {groupedLogs[date].map((log) => (
              <div 
                ...
                className={`group relative border rounded-2xl p-5 transition-all duration-300 cursor-pointer select-none overflow-hidden bg-zinc-900 hover:bg-zinc-800 border-zinc-800 hover:border-zinc-700 active:scale-[0.98]`}
              >
                ...
                <div className="flex items-start justify-between gap-4">
                  <div className="flex-1">
                    <div className="flex items-center gap-3 mb-2.5">
                       <span className={`text-xs font-black tracking-widest px-2 py-0.5 rounded uppercase ${log.type === 'LOSS' ? 'text-red-500 bg-red-500/10' : 'text-yellow-500 bg-yellow-500/10'}`}>

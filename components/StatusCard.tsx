...
const StatusCard: React.FC<StatusCardProps> = ({ isActive, timeLeft, schedule, blockStats, onToggle, onManualEntry, theme = 'dark' }) => {
    ...
    return (
        <div className={`w-full border rounded-3xl p-6 relative overflow-hidden group transition-colors duration-300 ${isDark ? 'bg-zinc-900 border-zinc-800' : 'bg-white border-zinc-200 shadow-sm'}`}>
            ...
            <div className="relative z-10 flex flex-col gap-5">
                ...
                <div className="flex justify-between items-end">
                    <div>
                        <div className={`text-7xl font-mono font-bold tracking-tighter leading-none transition-colors duration-300 ${isActive ? 'text-green-500' : isDark ? 'text-white/10' : 'text-zinc-200'}`}>
                    ...
                    <div className="flex items-end gap-3">
                        <button
                            ...
                            className={`w-16 h-16 rounded-2xl flex items-center justify-center transition-all duration-300 active:scale-95 border group/btn relative overflow-hidden ${isActive

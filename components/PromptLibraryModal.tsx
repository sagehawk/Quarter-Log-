...
const PromptLibraryModal: React.FC<PromptLibraryModalProps> = ({ isOpen, onClose, logsText, onCopySuccess }) => {
  ...
  useEffect(() => {
    if (isOpen) {
        ...
        try {
            ...
            // Combine defaults (excluding 'raw' which is hardcoded in rendering if needed, but here we keep it in list)
            // Filter out old defaults if they exist in local storage to avoid dupes
  ...
  const handleSavePrompt = (e: React.FormEvent) => {
    ...
    // Filter out existing custom prompts to append the new one
      ...
      <div className="relative w-full max-w-lg bg-slate-900 border-2 border-white/10 rounded-3xl shadow-2xl overflow-hidden flex flex-col max-h-[85vh] animate-slide-up">
        ...
        <div className="p-6 border-b border-white/5 bg-slate-900 z-10">
            <div className="flex justify-between items-center">
                <div>
                    <h2 className="text-2xl font-black text-white uppercase tracking-tight italic">Prompt Library</h2>
                    <p className="text-slate-400 text-[10px] font-bold uppercase tracking-wide">
                        Choose a prompt to copy with your logs
                    </p>
                </div>
                <button 
        ...
        <div className="overflow-y-auto p-4 space-y-3 custom-scrollbar">
            ...
            {prompts.map((prompt) => (
                <button
                    ...
                    className="w-full text-left group relative bg-slate-800/40 hover:bg-slate-800 border-2 border-transparent hover:border-brand-500/30 rounded-xl p-4 transition-all duration-200"
                >
